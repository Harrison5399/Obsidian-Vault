...
- **Gaussian Distribution:**
  \[
  P(x \mid \mu,\sigma)=\frac{1}{\sigma\sqrt{2\pi}}\,\exp\!\Bigl(-\frac{(x-\mu)^2}{2\sigma^2}\Bigr)
  \]
...
\[
\hat{w}_{LS} = \arg\min_w \sum_{i=1}^n \bigl(y_i - x_i^T w\bigr)^2,
\]
with closed-form solution (if \(X^TX\) is invertible):
\[
...
### Objective
\[
\hat{w}_{\text{ridge}} = \arg\min_w \sum_{i=1}^n \bigl(y_i - x_i^T w\bigr)^2 + \lambda \|w\|_2^2.
\]
...
### Objective
\[
\hat{w}_{\text{lasso}} = \arg\min_w \sum_{i=1}^n \bigl(y_i - x_i^T w\bigr)^2 + \lambda \|w\|_1.
\]
- **Key Feature:**  
...
\[
p(w) \propto \exp\!\Bigl(-\frac{\lambda}{2}\|w\|_2^2\Bigr),
\]
